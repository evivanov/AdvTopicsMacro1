...
k = fsolve(@(x) F(x), k_star_new*ones(201,1));
plot(1:T, k(1:T))
...
% ki1 = 0;
% 
% while abs(lower_bound_guess_k_t_1 - upper_bound_guess_k_t_1) > 1e-7
%     ki = k(T-1);
%     k(1:T-2) = zeros(T-3+1,1);
%     c(1:T-2) = zeros(T-3+1,1);
%     for t = T-2:-1:1
%        c(t) = (1/beta)*c(t+1)/( z1*alpha*k(t+1)^(alpha-1) + 1 - delta);
%        func = @(x) (k(t+1) - z1*x.^alpha - (1-delta).*x + c(t));
%        k(t) = lsqnonlin(func, k(t+1), lower_bound_guess_k_t_1, upper_bound_guess_k_t_1);
%        if k(t) < k_star
